        ...
        <div className="min-h-screen bg-gray-50 px-6 py-10">
            ...
            {loading ? (
                ...
                <div className="space-y-4 max-w-4xl mx-auto">
                    {invoices.map((inv) => {
                        const isClient = user?.id === inv.client.id;
                        return (
                            <div
                                key={inv.id}
                            ...
                            >
                                <div>
                                    ...
                                    <p className="font-semibold text-gray-800">
                                        Service: {inv.reservation?.service?.name || '—'}
                                    </p>
                                    <p className="text-gray-600 text-sm">
                                        Role: <span className={`font-medium ${isClient ? 'text-blue-600' : 'text-green-600'}`}>
                      {isClient ? 'Client' : 'Provider'}
                    </span>
                                    </p>
                                    <p className="text-sm text-gray-500">Date: {new Date(inv.createdAt).toLocaleString()}</p>
                                </div>
